...
(function (window, document) {
	...
	fallback.spawn.instance = function(library, urls) {
		...
		element.onload = function() {
			...
			// Blackbaud edit. WI # 655448.
			// This is causing issues with IE11. Fallback is showing that the sky css failed to load even
			// though the network tab shows that it succeeded with a 200 response. Because fallback thinks
			// that the sky_css failed to load, it won't load our app's css file, which depends on the sky_css.
